...
class PreProcess:
    ...
    def __init__(self, config, device="cpu", need_parser=True):
        ...
        xs = xs[None].repeat(config.landmark_points, axis=0)
        ys = ys[None].repeat(config.landmark_points, axis=0)
    ...
    def process(self, mask, lms, device="cpu"):
        diff = to_var(
            (self.fix - torch.tensor(lms.transpose((1, 0)
    ...
    def __call__(self, image: Image):
        ...
        image, face = futils.dlib.crop(image, face_on_image)
        ...
        return [real, mask, diff], face_on_image
