...
class Solver_makeupGAN(Track):
    def __init__(self, config, device="cpu", data_loader=None, inference=False):
        ...
        self.result_path = osp.join(config.vis_path, config.task_name)
        self.snapshot_path = osp.join(config.snapshot_path, config.task_name)
        ...
        self.log_point = self.start_time
        self.log_step = config.log_step // torch.cuda.device_count()
        self.vis_step = config.vis_step // torch.cuda.device_count()
        self.snapshot_step = config.snapshot_step // torch.cuda.device_count()

        self.device = device

        self.build_model()
        super(Solver_makeupGAN, self).__init__()
    ...
    def train(self):
        ...
        for self.e in range(start, self.num_epochs):
            for self.i, (source_input, reference_input) in enumerate(self.data_loader_train):
                ...
                self.track("Discriminator forward")
                ...
                if (self.i) % self.vis_step == 0:
                    ...
                    self.vis_train([image_s, image_r, fake_A, fake_A, rec_A, rec_A])
